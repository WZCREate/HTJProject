"""
this file is used to remove some backup csv files and databales in DB (PROD)
"""
...
def main():
    # 1. 获取配置路径
    _, qprod_config_dir, root_dir = find_config_path()
    # 2. 加载QA配置
    config = load_config(qprod_config_dir)
    
    # 设置日志
    log_dir = Path("CommonFunc/Logs")
    log_dir.mkdir(parents=True, exist_ok=True)
    logger = set_log(config, "clean_prod.log", prefix="PROD")
    
    try:
        # 处理CSV文件
        csv_dir = os.path.join(root_dir, "PROD", "CSVs")
