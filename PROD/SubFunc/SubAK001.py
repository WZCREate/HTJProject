...
def save_filter_result(cursor, config, filter_name, input_count, output_count, 
    ...
    try:
        ...
        logger.info_print(f"过滤结果已保存到数据库")
        return True
...
def main():
    ...
    logger = set_log(config, "AK001.log", prefix="PROD")
    logger.info("开始执行 SubAK001 函数")
    csv_config = config["CSVs"]
    input_file = f"{root_dir}/PROD/{csv_config['MainCSV']}"
    output_file = f"{root_dir}/PROD/{csv_config['Filters']['Input']}"
...
if __name__ == "__main__":
    try:
        output_file = main()
        print(f"PROD: 初次过滤完成, 已写入 {output_file}.")
    except Exception as e:
        print(f"PROD: 处理失败：{str(e)}")